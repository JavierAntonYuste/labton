...
var Leaderboard = /** @class */ (function (_super) {
    ...
    Leaderboard.prototype.getData = function () {
        ...
        subject_id=document.getElementById('subject_id').getAttribute('value');

        fetch('/getTopPoints?subject_id='+subject_id)
        .then(results =>{
          return results.json();
        }).then(output =>{
          leaders=[]
...
if (document.getElementById('podiumSubject')!=null){
  ReactDOM.render(React.createElement(Leaderboard, null), document.getElementById('podiumSubject'));
}
