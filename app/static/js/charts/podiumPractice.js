...
var Leaderboard = /** @class */ (function (_super) {
    ...
    Leaderboard.prototype.getData = function () {
        ...
        practice_id=document.getElementById('practice_id').getAttribute('value');

        fetch('/getTopPoints?practice_id='+practice_id)
        .then(results =>{
          return results.json();
        }).then(output =>{
          leaders=[]
          for (i=0; i<output.length;i++){
            leaders.push({id: output[i][0], name: output[i][1], score: output[i][2]})
          }
...
if (document.getElementById('podiumPractice')!=null){
  ReactDOM.render(React.createElement(Leaderboard, null), document.getElementById('podiumPractice'));
